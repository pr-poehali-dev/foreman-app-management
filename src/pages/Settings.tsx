...
export default function Settings({ user, onLogout }: Props) {
  ...
  return (
    <div className="animate-fade-in max-w-2xl">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl sm:text-3xl font-oswald font-bold text-white tracking-wide">НАСТРОЙКИ</h1>
        <p className="text-sm mt-0.5" style={{ color: "var(--text-secondary)" }}>Профиль и параметры приложения</p>
      </div>

      {/* Profile card */}
      <div className="app-card p-4 sm:p-5 flex items-center gap-4 mb-5">
        <div
        ...
        <div className="flex-1 min-w-0">
          <p className="text-white font-semibold text-base sm:text-lg truncate">{user.full_name}</p>
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>@{user.login}</p>
          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
            <span
              className="status-badge text-xs"
              style={{
                background: user.role === 'manager' ? 'rgba(255,140,0,0.12)' : 'rgba(59,130,246,0.12)',
                color: user.role === 'manager' ? 'var(--orange)' : 'var(--blue)',
              }}>
      ...
      <div className="app-card overflow-hidden mb-4">
        <div className="px-4 sm:px-5 py-3" style={{ borderBottom: "1px solid var(--surface-4)" }}>
        ...
        <div className="divide-y" style={{ borderColor: "var(--surface-4)" }}>
          {[
            ...
            <div key={item.k} className="px-4 sm:px-5 py-4 flex items-center gap-3">
              ...
              {/* Toggle — w-12 h-7 for easier tap */}
              <button
                ...
                <span
                  ...
                  style={{ left: notifs[item.k] ? 'calc(100% - 24px)' : '4px' }}
                />
              </button>
            </div>
          ))}
      ...
      <div className="app-card overflow-hidden mb-4">
        ...
        <div className="divide-y" style={{ borderColor: "var(--surface-4)" }}>
          <div className="px-4 sm:px-5 py-4 flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "var(--surface-3)" }}>
            ...
            <button
              ...
              <span
                className="absolute top-1 w-5 h-5 rounded-full bg-white transition-all"
                style={{ left: notifs.autoSync ? 'calc(100% - 24px)' : '4px' }}
              />
            </button>
          </div>
          {[
            ...
            <div key={item.label} className="px-4 sm:px-5 py-4 flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "var(--surface-3)" }}>
                <Icon name={item.icon} fallback="Circle" size={15} style={{ color: "var(--orange)" }} />
              </div>
              <span className="flex-1 text-sm text-white">{item.label}</span>
              <span className="text-xs sm:text-sm flex-shrink-0" style={{ color: "var(--text-muted)" }}>{item.val}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* About */}
      <div className="app-card p-4 flex items-center gap-3 mb-5">
      ...
      {/* Logout — full width on mobile */}
      <div className="pb-4">
        <button
          onClick={onLogout}
          className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 rounded-xl text-sm font-medium transition-all min-h-[44px]"

    ...
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: "var(--surface-1)" }}>
      ...
      <div className="flex items-center gap-3 px-4 py-3 flex-shrink-0"
        ...
        <div>
          ...
          <p className="text-xs" style={{ color: "var(--text-muted)" }}>ИП МАСАЛОВ · {messages.length} сообщений</p>
        </div>
              ...
              <div key={m.id}>
                {showDate && (
                  <div className="flex items-center gap-3 my-3">
                    ...
                    <span className="text-xs px-3 py-1 rounded-full" style={{ background: "var(--surface-3)", color: "var(--text-muted)" }}>
                ...
                <div className={`flex gap-2 mb-2 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className="w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold flex-shrink-0 self-end"
                    style={{
                  ...
                  <div className={`max-w-[75%] ${isMe ? 'items-end' : 'items-start'} flex flex-col`}>
                    <div className="flex items-center gap-2 mb-1 px-1">
                      ...
                      {m.role === 'manager' && (
                        <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: "rgba(255,140,0,0.15)", color: "var(--orange)", fontSize: 10 }}>
                          управленец
                        </span>
                      )}
      ...
      {/* Input */}
      <form onSubmit={send} className="flex-shrink-0 px-4 py-3 flex gap-2"
        style={{ background: "var(--surface-2)", borderTop: "1px solid var(--surface-4)" }}>
        <input
          value={text}
